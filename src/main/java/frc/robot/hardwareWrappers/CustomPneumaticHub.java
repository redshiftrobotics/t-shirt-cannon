...
/**
 * Module class for controlling a REV Robotics Pneumatic Hub. Taken from real code, overridden for
 * custom pressure switch
 */
public class CustomPneumaticHub implements PneumaticsBase {
  ...
  /** Converts volts to PSI per what Spencer said. */
  private static double voltsToPsi(double sensorVoltage, double supplyVoltage) {
    sensorVoltage = sensorVoltage / supplyVoltage * 5;
    return MathUtil.interpolate(0, 150, MathUtil.inverseInterpolate(.5, 4.5, sensorVoltage));
  }

  /** Converts PSI to voltage per what Spencer said. */
  private static double psiToVolts(double pressure, double supplyVoltage) {
    double result = MathUtil.interpolate(.5, 4.5, MathUtil.inverseInterpolate(0, 150, pressure));
    return result / 5 * supplyVoltage;
  }
   ...
   */
  public CustomPneumaticHub(int module) {
  ...
  public void enableCompressorAnalog(double minPressure, double maxPressure) {
    ...
    // SmartDashboard.putNumber("Min Voltage", minAnalogVoltage);
    // SmartDashboard.putNumber("Max Voltage", maxAnalogVoltage);
